...
def get_leather_type(product_name):
    # 革の種類のキーワードリスト
    leather_keywords = [
        "SAMPL", "VIN BR", "HORWEEN/BRI", "BR/RUS", "PLAITED", "VTC BADALASSI",
        "BADALASSI", "NATUR/BR", "NATUR", "CAN/TUS", "TUS", "RUSSET", "PLAI",
        "FIX LEATHER", "WEBBNG", "WEBBING", "PASTURE SUEDE", "STIRRUP", "LEATHER BALM", "BRI"
    ]
...
def upload_product_excel():
    ...
    for _, row in df.iterrows():
        try:
            cursor.execute("""
                INSERT INTO products (商品名, 品番CD, カラーNO, カラー名, サイズ数計, 革の種類)
                VALUES (?, ?, ?, ?, ?, ?)
...
def download_sales_summary():
    ...
    # 2. 売上が0の場合は0を入れる（再度fillna(0)で全体に適用）
    ...
    # 重複判定キー（全カラムで完全一致ではなく、全体の行で直前と同じならnan化）
    ...
    # ファイル名と保存先
    folder = os.path.join(os.path.expanduser("~"), "Downloads")
    filename = f"商品別売上集計_{start_year_var.get()}年{start_month_var.get()}月-{end_year_var.get()}年{end_month_var.get()}月.xlsx"
    full_path = os.path.join(folder, filename)
...
tk.Label(window, text="WEB用のExcelファイルを選択してアップロード", font=("Arial", 10)).pack(pady=5)
tk.Button(window, text="ファイルをアップロード", command=upload_file).pack(pady=10)
tk.Button(window, text="複数ファイルをアップロード", command=upload_files).pack(pady=10)

tk.Label(window, text="店舗用Excelファイルを選択してアップロード", font=("Arial", 10)).pack(pady=5)
tk.Button(window, text="ファイルをアップロード", command=upload_fukuoka_file).pack(pady=10)
tk.Button(window, text="複数ファイルをアップロード", command=upload_fukuoka_files).pack(pady=10)
...
tk.Label(window, text="【商品マスタ管理】", font=("Arial", 10, "bold")).pack(pady=10)
tk.Button(window, text="商品マスタExcel取込", command=upload_product_excel).pack(pady=5)
tk.Button(window, text="商品マスタ金額CSV取込", command=upload_price_csv).pack(pady=5)
